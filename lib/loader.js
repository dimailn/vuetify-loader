...
module.exports = async function (content, sourceMap) {
  ...
  acornWalk.simple(ast, {
    CallExpression (node) {
      ...
      if (
        ...
        //   ? node.callee.name === '_createVNode'
        //   : (node.callee.type === 'MemberExpression' &&
        //     node.callee.object.name === '_vm' &&
        //     node.callee.property.name === '_createVNode')
        node.callee.type === 'Identifier'
        && node.callee.name === '_resolveComponent'
      ) {
        ...
        node.callee.type === 'MemberExpression' &&
          node.callee.object.name === '_vm' &&
          ['_mergeProps', '_toHandlers'].includes(node.callee.property.name) &&
  ...
  if (imports.length) {
    importsStr = imports.map(v => v[1]).join('\n') + '\n\n' + importsStr
  }
